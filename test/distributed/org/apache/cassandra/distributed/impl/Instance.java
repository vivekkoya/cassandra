...
public class Instance extends IsolatedExecutor implements IInvokableInstance {
    ...
    public static void setReceiveMessagesAsync(boolean v) {
        RECEIVE_MESSAGES_ASYNC = v;
    }
    ...
    Instance(
        ...
        super(
            ...
            classLoader,
            executorFactory()
                .configurePooled("isolatedExecutor", Integer.MAX_VALUE)
        ...
        this.config = config;
        if (fileSystem != null) File.unsafeSetFilesystem(fileSystem);
        Object clusterId = Objects.requireNonNull(
            config.get(Constants.KEY_DTEST_API_CLUSTER_ID),
            "cluster_id is not defined"
        );
        ClusterIDDefiner.setId("cluster-" + clusterId);
        InstanceIDDefiner.setInstanceId(config.num());
        FBUtilities.setBroadcastInetAddressAndPort(
            InetAddressAndPort.getByAddressOverrideDefaults(
                config.broadcastAddress().getAddress(),
    ...
    public LogAction logs() {
        ...
        File f = new File(
            ...
            String.format(
                "build/test/logs/%s/%s/%s/%s/system.log",
                tag,
                suite,
                clusterId,
                instanceId
        ...
        if (!f.exists()) f = new File(
            FileSystems.getDefault(),
            String.format(
                "build/test/logs/%s/%s/%s/system.log",
                tag,
                clusterId,
                instanceId
            )
        ...
        if (!f.exists()) throw new AssertionError(
            "Unable to locate system.log under " +
            f.absolutePath() +
            "; make sure ICluster.setup() is called or extend TestBaseImpl and do not define a static beforeClass function with @BeforeClass"
        );
    ...
    @Override
    public SimpleQueryResult executeInternalWithResult(
        String query,
        Object... args
    ) {
    ...
    public static SimpleQueryResult unsafeExecuteInternalWithResult(
        String query,
        Object... args
    ) {
        ...
        try {
            QueryHandler.Prepared prepared = QueryProcessor.prepareInternal(
                query
            );
            ...
            if (result != null) result.setWarnings(
    ...
    public void schemaChangeInternal(String query) {
        sync(() -> {
            try {
                ...
                CQLStatement statement = QueryProcessor.parseStatement(
                    query,
                ...
                throw new RuntimeException(
                    "Error setting schema for test (query was: " + query + ")",
                    e
                );
            }
        }).run();
    ...
    protected void registerMockMessaging(ICluster<?> cluster) {
        MessagingService.instance()
            .outboundSink.add((message, to) -> {
                if (!internodeMessagingStarted) {
                    inInstancelogger.debug(
                        "Dropping outbound message {} to {} as internode messaging has not been started yet",
                        message,
                        to
                    );
                    return false;
                }
                cluster.deliverMessage(
                    to,
                    serializeMessage(message.from(), to, message)
                );
                return false;
            });
    }
    ...
    protected void registerInboundFilter(ICluster<?> cluster) {
        MessagingService.instance()
            .inboundSink.add(message -> {
                if (!cluster.filters().hasInbound()) return true;
                if (isShutdown()) return false;
                IMessage serialized = serializeMessage(
                    message.from(),
                    toCassandraInetAddressAndPort(broadcastAddress()),
                    message
                );
                ...
                return cluster
                    .filters()
                    .permitInbound(fromNum, toNum, serialized);
            });
    }
    ...
    protected void registerOutboundFilter(ICluster cluster) {
        MessagingService.instance()
            .outboundSink.add((message, to) -> {
                if (isShutdown()) return false; // TODO: Simulator needs this to trigger a failure
                int fromNum = config.num(); // since this instance is sending the message, from will always be this instance
                IInstance toInstance = cluster.get(
                    fromCassandraInetAddressAndPort(to)
                );
                if (toInstance == null) return true; // TODO: Simulator needs this to trigger a failure
                if (
                    !cluster.filters().hasInbound() &&
                    !cluster.filters().hasOutbound()
                ) return true; // no filters... nothing to see here
                int toNum = toInstance.config().num();
                IMessage serialzied = serializeMessage(
                    message.from(),
                    to,
                    message
                );
                return cluster
                    .filters()
                    .permitOutbound(fromNum, toNum, serialzied);
            });
    }
    ...
    public static IMessage serializeMessage(
        ...
        if (
            fromVersion < MessagingService.current_version &&
            ((messageOut.verb().serializer() ==
                        ((IVersionedAsymmetricSerializer) NoPayload.serializer) ||
        ...
        try (DataOutputBuffer out = new DataOutputBuffer(1024)) {
            ...
            if (
                fromVersion >= MessagingService.VERSION_40 &&
                messageOut.verb().id == BATCH_STORE_REQ.id
            ) {
            ...
            if (
                ...
                String.format(
                    "Message serializedSize(%s) does not match what was written with serialize(out, %s) for verb %s and serializer %s; " +
                    "expected %s, actual %s ",
                    toVersion,
                    toVersion,
                    messageOut.verb(),
                    messageOut.verb().serializer().getClass(),
                    messageOut.serializedSize(toVersion),
                    bytes.length
                )
            );
            return new MessageImpl(
    ...
    public static Message<?> deserializeMessage(IMessage message) {
        try (DataInputBuffer in = new DataInputBuffer(message.bytes())) {
            return Message.serializer.deserialize(
                in,
                toCassandraInetAddressAndPort(message.from()),
                message.version()
            );
        } catch (Throwable t) {
            throw new RuntimeException(
                "Can not deserialize message " + message,
                t
            );
        }
    }
    ...
    public static Message.Header deserializeHeader(IMessage message) {
        try (DataInputBuffer in = new DataInputBuffer(message.bytes())) {
            return Message.serializer.deserializeHeader(
                in,
                toCassandraInetAddressAndPort(message.from()),
                message.version()
            );
            ...
            throw new RuntimeException(
                "Can not deserialize heaader " + message,
                t
            );
        }
    }
    ...
    public void receiveMessage(IMessage message) {
        if (RECEIVE_MESSAGES_ASYNC) async(
            receiveMessageRunnable(message)
        ).apply(false);
    ...
    public void receiveMessageWithInvokingThread(IMessage message) {
        if (
            ...
            "Must be invoked by a Thread utilising the node's class loader"
        );
        receiveMessageRunnable(message).accept(true);
    }

    private SerializableConsumer<Boolean> receiveMessageRunnable(
        IMessage message
    ) {
        return runOnCaller -> {
            if (!internodeMessagingStarted) {
                inInstancelogger.debug(
                    "Dropping inbound message {} to {} as internode messaging has not been started yet",
                    message,
            ...
            if (message.version() > MessagingService.current_version) {
                throw new IllegalStateException(
                    String.format(
                        "Node%d received message version %d but current version is %d",
                        this.config.num(),
                        message.version(),
            ...
            if (runOnCaller) {
                ...
                if (executor.isShutdown()) {
                    MessagingService.instance()
                        .metrics.recordDroppedMessage(
                            messageIn,
                            messageIn.elapsedSinceCreated(TimeUnit.NANOSECONDS),
                            TimeUnit.NANOSECONDS
                        );
                    inInstancelogger.warn(
                        "Dropping message {} due to stage {} being shutdown",
                        messageIn,
                        header.verb.stage
                    );
                    return;
                ...
                // place for it to run
                if (executor == ImmediateExecutor.INSTANCE) executor =
                    Stage.MISC.executor();
                executor.execute(ExecutorLocals.create(state), () ->
                    MessagingService.instance().inboundSink.accept(messageIn)
                );
            }
        };
    ...
    public void setMessagingVersion(InetSocketAddress endpoint, int version) {
        if (
            ...
            .versions.set(toCassandraInetAddressAndPort(endpoint), version);
        else inInstancelogger.warn(
            "Skipped setting messaging version for {} to {} as daemon not initialized yet. Stacktrace attached for debugging.",
            endpoint,
            version,
    ...
    private DurationSpec startupTimeout() {
        ...
        if (c instanceof String) return new DurationSpec.LongNanosecondsBound(
            (String) c
        );
        if (c instanceof Number) return new DurationSpec.LongNanosecondsBound(
            ((Number) c).longValue()
        );
        throw new IllegalArgumentException(
            "Key " +
            Constants.KEY_DTEST_STARTUP_TIMEOUT +
            " only allowed to have string/number values, but given " +
            c +
            ": " +
            c.getClass()
        );
    ...
    public void startup(ICluster cluster) {
        ...
        assert startedAt.compareAndSet(0L, System.nanoTime()) : String.format(
            "startedAt on instance %d expected to be 0, but was %d",
            config().num(),
            startedAt.get()
        );
        ...
        Future<?> result = async(() -> {
            ...
            try {
                boolean isFullStartup =
                    config.get(Constants.KEY_DTEST_FULL_STARTUP) != null &&
                    (boolean) config.get(Constants.KEY_DTEST_FULL_STARTUP);
                if (isFullStartup) {
                    assert config
                        .networkTopology()
                        .contains(config.broadcastAddress()) : String.format(
                        "Network topology %s doesn't contain the address %s",
                        config.networkTopology(),
                        config.broadcastAddress()
                    );
                    DistributedTestInitialLocationProvider.assign(
                        config.networkTopology()
                    );
    ...
    private static void propagateMessagingVersions(ICluster cluster) {
        cluster
            ...
            .forEach(reportToObj -> {
                ...
                if (reportTo.isShutdown()) return;

                int reportToVersion = reportTo.getMessagingVersion();
                if (reportToVersion == 0) return;

                cluster
                    .stream()
                    .forEach(reportFromObj -> {
                        IInstance reportFrom = (IInstance) reportFromObj;
                        if (
                            reportFrom == reportTo || reportFrom.isShutdown()
                        ) return;
                        ...
                        if (
                            reportFromVersion == 0
                        ) return; // has not read configuration yet, no accessing messaging version
    ...
    protected void partialStartup(ICluster<?> cluster)
        throws IOException, NoSuchFieldException, IllegalAccessException, ExecutionException, InterruptedException, StartupException {
        ...
        if (config.has(JMX)) setupMbeanWrapper();
        DatabaseDescriptor.daemonInitialization();
        if (config.has(JMX)) startJmx();
        ...
        CassandraDaemon.disableAutoCompaction(
        ...
        CassandraDaemon.disableAutoCompaction(
            Schema.instance.distributedKeyspaces().names()
        );
        ...
        JVMStabilityInspector.replaceKiller(
            new InstanceKiller(Instance.this::shutdown)
        );

        StorageService.instance.registerDaemon(
        ...
        if (config.has(GOSSIP)) {
            try {
                ...
                if (
                    ...
                    "Can't assign requested address".equals(cause.getMessage())
                ) throw new RuntimeException(
                    "Unable to bind, run the following in a termanl and try again:\nfor subnet in $(seq 0 5); do for id in $(seq 0 5); do sudo ifconfig lo0 alias \"127.0.$subnet.$id\"; done; done;",
                    e
                );
                throw e;
            }
            ...
            boolean joinRing =
                config.get(Constants.KEY_DTEST_JOIN_RING) == null ||
                (boolean) config.get(Constants.KEY_DTEST_JOIN_RING);
            if (
                ...
                if (config.has(BLANK_GOSSIP)) peers.forEach(peer ->
                    GossipHelper.statusToBlank(
                        (IInvokableInstance) peer
                    ).accept(this)
                );
                else if (cluster instanceof Cluster) peers.forEach(peer ->
                    GossipHelper.statusToNormal(
                        (IInvokableInstance) peer
                    ).accept(this)
                );
        ...
        if (
            !FBUtilities.getBroadcastAddressAndPort()
                .getAddress()
                .equals(broadcastAddress().getAddress()) ||
    ...
    public void postStartup() {
        sync(() -> StorageService.instance.doAuthSetup(false)).run();
    }

    protected void mkdirs() {
        new File(
            config.getString("saved_caches_directory")
        ).tryCreateDirectories();
        new File(config.getString("hints_directory")).tryCreateDirectories();
        new File(
            config.getString("commitlog_directory")
        ).tryCreateDirectories();
        for (String dir : (String[]) config.get(
    ...
    private Config loadConfig(IInstanceConfig overrides) {
        ...
        boolean check = true;
        if (overrides.get(Constants.KEY_DTEST_API_CONFIG_CHECK) != null) check =
    ...
    public Future<Void> shutdown(boolean runOnExitThreads) {
        return shutdown(
            runOnExitThreads,
            TEST_JVM_SHUTDOWN_MESSAGING_GRACEFULLY.getBoolean()
        );
    }

    public Future<Void> shutdown(
        boolean runOnExitThreads,
        boolean shutdownMessagingGracefully
    ) {
        Future<?> future = async((ExecutorService executor) -> {
            Throwable error = null;
            inInstancelogger.warn(
                "Shutting down in thread {}",
            ...
            ThrowingRunnable shutdownBatchlogAndHints = () -> {
                ...
                try {
                    ...
                    if (
                        !"HintsService has already been shut down".equals(
                                e.getMessage()
                            )
                    ) throw e;
                }
            ...
            error = parallelRun(
                ...
                () ->
                    AbstractAllocatorMemtable.MEMORY_POOL.shutdownAndWait(
                        1L,
                        MINUTES
                    ),
                () ->
                    DiagnosticSnapshotService.instance.shutdownAndWait(
                        1L,
                        MINUTES
                    ),
            ...
            error = parallelRun(
                ...
                () -> {
                    if (config.has(NETWORK)) {
                        try {
                            GlobalEventExecutor.INSTANCE.awaitInactivity(
                                1L,
                                MINUTES
                            );
            ...
            error = parallelRun(error, executor, () ->
                shutdownAndWait(
                    Collections.singletonList(JMXBroadcastExecutor.executor)
                )
            );
    ...
    private void withThreadLeakCheck() {
        ...
        threadSet
            .stream()
            .filter(t -> t.getContextClassLoader() == classLoader)
            .forEach(t -> {
                StringBuilder sblocal = new StringBuilder(
                    "\nUnterminated thread detected " +
                    t.getName() +
                    " in group " +
                ...
                if (
                    t instanceof
                    NamedThreadFactory.InspectableFastThreadLocalThread
                ) {
                    sblocal.append("\nCreation Stack Trace:");
                    for (StackTraceElement stackTraceElement : ((NamedThreadFactory.InspectableFastThreadLocalThread) t).creationTrace) sblocal
                        .append("\n\t\t\t")
        ...
        if (!msg.isEmpty()) throw new RuntimeException(msg);
    }

    @Override
    public int liveMemberCount() {
        if (!initialized || isShutdown()) return 0;
    ...
    private static class CapturingOutput implements Closeable {
        ...
        private final ByteArrayOutputStream outBase =
            new ByteArrayOutputStream();
        private final ByteArrayOutputStream errBase =
            new ByteArrayOutputStream();
    ...
    public static class DTestNodeTool
        extends NodeTool
        implements AutoCloseable {

        private final StorageServiceMBean storageProxy;
        private final CollectingNotificationListener notifications =
            new CollectingNotificationListener();
        ...
        public int execute(String... args) {
            try {
                ...
                try {
                    storageProxy.removeNotificationListener(
                        notifications,
                        null,
                        null
                    );
        ...
        protected void err(Throwable e) {
            if (e instanceof SystemExitException) return;
            super.err(e);
            latestError = e;
        }
    ...
    private static final class CollectingNotificationListener
    ...
    private static Throwable parallelRun(
