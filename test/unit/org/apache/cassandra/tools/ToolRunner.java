...
public class ToolRunner {
    ...
    public static ObservableTool invokeAsync(
        Map<String, String> env,
        String... args
    ) {
    ...
    public static ToolResult invokeClass(
        String klass,
        InputStream stdin,
        String... args
    ) {
    ...
    public static ToolResult invokeNodetoolJvmDtest(
        IInstance node,
        String... args
    ) {
    ...
    public static <T> Pair<T, ToolResult> invokeSupplier(
        Supplier<T> runMe,
        InputStream stdin
    ) {
    ...
    public static final class ToolResult {
        ...
        private void assertExitCode(int code) {
            if (code != 0) fail(
                String.format(
                    "%s%nexited with code %d%nstderr:%n%s%nstdout:%n%s",
                    argsToLogString(),
                    code,
                    getStderr(),
                    getStdout()
        ...
        public String getCleanedStderr(List<String> regExpCleaners) {
            ...
            for (String regExp : regExpCleaners) sanitizedStderr =
                sanitizedStderr.replaceAll(regExp, "");
        ...
        public final class AssertHelp {
            ...
            public AssertHelp success() {
                if (exitCode != 0) fail("was not successful");
                return this;
            }
            ...
            public AssertHelp errorContainsAny(String... messages) {
                ...
                if (!Stream.of(messages).anyMatch(stderr::contains)) fail(
                    "stderr does not contain " + Arrays.toString(messages)
                );
                return this;
            }
    ...
    private static final class ForkedObservableTool implements ObservableTool {
        ...
        private ForkedObservableTool(
            ...
            if (includeStdinWatcher) numWatchers = 3;
            ioWatchers = new Thread[numWatchers];
            ioWatchers[0] = new Thread(
                new StreamGobbler<>(process.getErrorStream(), err, false)
            );
            ioWatchers[0].setDaemon(true);
            ioWatchers[0].setName("IO Watcher stderr");
            ioWatchers[0].start();

            ioWatchers[1] = new Thread(
                new StreamGobbler<>(process.getInputStream(), out, false)
            );
            ...
            if (includeStdinWatcher) {
                ioWatchers[2] = new Thread(
                    new StreamGobbler<>(stdin, process.getOutputStream(), true)
                );
        ...
        private void onComplete() throws InterruptedException {
            ...
            for (Thread t : ioWatchers) t.join();
        }
