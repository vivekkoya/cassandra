...
public class UFSecurityTest extends CQLTester {
    ...
    public void testSecurityPermissions() throws Throwable {
        ...
        for (String source : cfnSources) {
            assertInvalidMessage(
                "Java UDF validation failed: [call to java.lang.Class.forName()]",
        ...
        for (String[] typeAndSource : typesAndSources) {
            assertInvalidMessage(
                typeAndSource[0] + " cannot be resolved",
                "CREATE OR REPLACE FUNCTION " +
                KEYSPACE +
                ".invalid_class_access(val double) " +
                "RETURNS NULL ON NULL INPUT " +
    ...
    private static void assertAccessControlException(
        String script,
    ...
    public void testAmokUDF() throws Throwable {
        ...
        for (int i = 1; i <= maxTries; i++) {
            try {
                ...
                DatabaseDescriptor.setUserFunctionTimeoutPolicy(
                    Config.UserFunctionTimeoutPolicy.ignore
                );
                ...
                // reset to defaults
                DatabaseDescriptor.setUserDefinedFunctionWarnTimeout(
                    udfWarnTimeout
                );
                DatabaseDescriptor.setUserDefinedFunctionFailTimeout(
                    udfFailTimeout
                );
            }
        }
