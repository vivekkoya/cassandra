...
public class HandshakeTest {
    ...
    static final InetAddressAndPort TO_ADDR =
        InetAddressAndPort.getByAddressOverrideDefaults(
            InetAddresses.forString("127.0.0.2"),
            7012
        );
    ...
    private Result handshake(int outMin, int outMax)
        throws ExecutionException, InterruptedException {
        return handshake(new AcceptVersions(outMin, outMax), null);
    }

    private Result handshake(int outMin, int outMax, int inMin, int inMax)
        throws ExecutionException, InterruptedException {
        return handshake(
            new AcceptVersions(outMin, outMax),
            new AcceptVersions(inMin, inMax)
        );
    }

    private Result handshake(
        AcceptVersions acceptOutbound,
        AcceptVersions acceptInbound
    ) throws ExecutionException, InterruptedException {
        InboundSockets inbound = new InboundSockets(
            new InboundConnectionSettings().withAcceptMessaging(acceptInbound)
        );
        try {
            inbound.open();
            InetAddressAndPort endpoint = inbound
                .sockets()
                .stream()
                .map(s -> s.settings.bindAddress)
                .findFirst()
                .get();
            EventLoop eventLoop = factory.defaultGroup().next();
            Future<Result<MessagingSuccess>> future = initiateMessaging(
                eventLoop,
                SMALL_MESSAGES,
    ...
    @Test
    public void testBothCurrentVersion()
    ...
    @Test
    public void testSendCompatibleOldVersion()
        throws InterruptedException, ExecutionException {
        Result result = handshake(
            current_version,
            current_version + 1,
            current_version + 1,
            current_version + 2
        ...
        Assert.assertEquals(
            current_version + 1,
            result.success().messagingVersion
        );
        result.success().channel.close();
    }

    @Test
    public void testSendCompatibleFutureVersion()
        throws InterruptedException, ExecutionException {
        Result result = handshake(
            MessagingService.minimum_version,
            current_version + 1
        );
        Assert.assertEquals(Result.Outcome.SUCCESS, result.outcome);
    ...
    @Test
    public void testSendIncompatibleFutureVersion()
        throws InterruptedException, ExecutionException {
    ...
    @Test
    public void testSendIncompatibleOldVersion()
        throws InterruptedException, ExecutionException {
        Result result = handshake(
            current_version + 1,
            current_version + 1,
    ...
    @Test
    public void testSendAllSupported()
        throws InterruptedException, ExecutionException {
        List<MessagingService.Version> supportedVersions =
            MessagingService.Version.supportedVersions();
        for (MessagingService.Version outMin : supportedVersions) for (MessagingService.Version outMax : supportedVersions) if (
            outMin.value <= outMax.value
        ) for (MessagingService.Version inMin : supportedVersions) for (MessagingService.Version inMax : supportedVersions) if (
            inMin.value <= inMax.value
        ) {
            Result result = handshake(
                outMin.value,
                outMax.value,
            ...
            boolean expectSuccess =
                outMin.value <= inMax.value && inMin.value <= outMax.value;

            Assert.assertEquals(
                String.format(
                    "wrong result outcome for outMin %s outMax %s inMin %s inMax %s",
                    outMin.value,
                    outMax.value,
                    inMin.value,
                    inMax.value
            ...
            if (expectSuccess) {
                Assert.assertEquals(
                    String.format(
                        "wrong agreed messagingVersion for outMin %s outMax %s inMin %s inMax %s",
                        outMin.value,
                        outMax.value,
                        inMin.value,
                        inMax.value
                    ),
                    Math.min(outMax.value, inMax.value),
                    result.success().messagingVersion
                );
                result.success().channel.close();
            }
    ...
    @Test
    public void testOutboundConnectionfFallbackDuringUpgrades()
        ...
        testOutboundFallbackOnSSLHandshakeFailure(
            ...
            true,
            SslFallbackConnectionType.NO_SSL,
            false
        );
    ...
    @Test
    public void testOutboundConnectionfFallbackDuringDowngrades()
        throws ClosedChannelException, InterruptedException {
    ...
    public void testOutboundConnectionDoesntFallbackWhenErrorIsNotSSLRelated()
        ...
        try {
            ...
            OutboundConnection outboundConnection = initiateOutbound(
                endpoint,
                SslFallbackConnectionType.SSL,
                true
            );
            ...
            while (
                outboundConnection.connectionAttempts() <
    ...
    private ServerEncryptionOptions getServerEncryptionOptions(
        ...
        serverEncryptionOptionsBuilder
            ...
            .withOutboundKeystorePassword(
                TlsTestUtils.SERVER_OUTBOUND_KEYSTORE_PASSWORD
            )
            ...
            .withSslContextFactory(
                (new ParameterizedClass(
                        DefaultSslContextFactory.class.getName(),
                        new HashMap<>()
                    ))
            );
    ...
    private InboundSockets getInboundSocket(
        ServerEncryptionOptions serverEncryptionOptions
    ) {
        InboundConnectionSettings settings = new InboundConnectionSettings()
            .withAcceptMessaging(
                new AcceptVersions(minimum_version, current_version)
            )
    ...
    private OutboundConnection initiateOutbound(
        InetAddressAndPort endpoint,
        SslFallbackConnectionType connectionType,
        boolean optional
        ...
        final OutboundConnectionSettings settings =
            new OutboundConnectionSettings(endpoint)
                ...
                .withDebugCallbacks(
                    new HandshakeAcknowledgeChecker(t -> handshakeEx = t)
                )
                .withFrom(FROM_ADDR);
        ...
        Message<GossipDigestSyn> message = Message.out(
    ...
    private void testOutboundFallbackOnSSLHandshakeFailure(
        SslFallbackConnectionType fromConnectionType,
        boolean fromOptional,
        SslFallbackConnectionType toConnectionType,
        boolean toOptional
    ) throws ClosedChannelException, InterruptedException {
        // Configures inbound connections to be optional mTLS
        InboundSockets inbound = getInboundSocket(
            getServerEncryptionOptions(toConnectionType, toOptional)
        );
        try {
            InetAddressAndPort endpoint = inbound
                .sockets()
    ...
    private static class HandshakeAcknowledgeChecker
        implements OutboundDebugCallbacks {
        ...
        @Override
        public void onSendSmallFrame(
            int messageCount,
            int payloadSizeInBytes
        ) {}

        @Override
        public void onSentSmallFrame(
            int messageCount,
            int payloadSizeInBytes
        ) {}

        @Override
        public void onFailedSmallFrame(
            int messageCount,
            int payloadSizeInBytes
        ) {}
        ...
        public void onConnect(
            ...
            if (acks.incrementAndGet() > 1) fail.accept(
                new AssertionError("Handshake was acknowledged more than once")
            );
        }
    }
}
