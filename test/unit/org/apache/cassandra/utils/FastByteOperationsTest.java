...
public class FastByteOperationsTest {
    ...
    // private static final FastByteOperations.PureJavaOperations PJO =
    //     new FastByteOperations.PureJavaOperations();
    ...
    private static int firstdiff(byte[] canon, byte[] test) {
        for (int i = 0; i < canon.length; i++) if (
            canon[i] != test[i]
        ) return i;
        return -1;
    }
    ...
    private void testTwiddleOneByteComparisons(
        byte[] bytes1,
        byte[] bytes2,
        int count,
        boolean start,
    ...
    private void testComparison(byte[] bytes1, byte[] bytes2) {
        // assert sameComparisonResult(
        //     // PJO.compare(bytes1, 0, bytes1.length, bytes2, 0, bytes2.length),
        //     UO.compare(bytes1, 0, bytes1.length, bytes2, 0, bytes2.length)
        // );
        // assert sameComparisonResult(
        //     // PJO.compare(
        //     //     bytes1,
    ...
    private void testComparison(ByteBuffer bytes1, byte[] bytes2) {
        // assert sameComparisonResult(
        //     // PJO.compare(bytes1, bytes2, 0, bytes2.length),
        //     UO.compare(bytes1, bytes2, 0, bytes2.length)
        // );
        // assert sameComparisonResult(
        //     // PJO.compare(bytes1, bytes2, 10, bytes2.length - 10),
        //     UO.compare(bytes1, bytes2, 10, bytes2.length - 10)
        // );
    }

    private void testComparison(ByteBuffer bytes1, ByteBuffer bytes2) {
        // assert sameComparisonResult(
        //     // PJO.compare(bytes1, bytes2),
        //     UO.compare(bytes1, bytes2)
        // );
    }
