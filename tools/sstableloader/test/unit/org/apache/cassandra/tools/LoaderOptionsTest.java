...
public class LoaderOptionsTest {
    ...
    public void testNativePort() throws Exception {
        ...
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
        );
        String[] args = {
            "-d",
            "127.9.9.1",
        ...
        config = new File(
            Paths.get(
                ...
                "test",
                "conf",
                "unit-test-conf/test-native-port.yaml"
            ).normalize()
        );
        String[] args2 = {
            "-d",
    ...
    public void testEncryptionSettingsOverride() throws Exception {
        // Default Cassandra config
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra-mtls.yaml").normalize()
        );
        String[] args = {
            "-d",
            "127.9.9.1",
        ...
        assertEquals(
            "truststorePass1",
            options.clientEncOptions.truststore_password
        );
        assertEquals("test.jks", options.clientEncOptions.keystore);
        assertEquals("testdata1", options.clientEncOptions.keystore_password);
        assertEquals(
            "TLS_RSA_WITH_AES_256_CBC_SHA",
            options.clientEncOptions.cipherSuitesArray()[0]
        );
    ...
    @Test
    public void testDeprecatedThrottleSettingsWithLongSettingNames()
        throws IOException {
        // Default Cassandra config
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
    ...
    @Test
    public void failsWhenThrottleSettingAndDeprecatedAreProvided()
        throws IOException {
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
        );
    ...
    @Test
    public void failsWhenThrottleSettingAndDeprecatedAreProvidedWithLongSettingNames()
        throws IOException {
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
        );
    ...
    @Test
    public void failsWhenInterDCThrottleSettingAndDeprecatedAreProvided()
        throws IOException {
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
        );
    ...
    @Test
    public void failsWhenInterDCThrottleSettingAndDeprecatedAreProvidedWithLongSettingNames()
        throws IOException {
        File config = new File(
            Paths.get(".", "test", "conf", "cassandra.yaml").normalize()
        );
    ...
    @Test
    public void testEntireSSTableSettingsWithLongSettingNames()
        throws IOException {
    ...
    public static String sstableDirName(String ks, String cf)
        throws IOException {
    ...
    public static File sstableDir(String ks, String cf) throws IOException {
        ...
        File[] cfDirs = ksDir.tryList(
            (dir, name) -> cf.equals(name) || name.startsWith(cf + '-')
        );
        return cfDirs[0];
    }
    ...
    public static File copySSTables() throws IOException {
        ...
        FileUtils.copyDirectory(
            new File(srcDir, "legacy_tables").toJavaIOFile(),
            new File(dataDir, "legacy_sstables").toJavaIOFile()
        );
        return dataDir;
    }
