...
abstract class FrameDecoderWith8bHeader extends FrameDecoder {
    ...
    protected void decode(
        ...
        try {
            if (stash != null) {
                if (!copyToSize(in, stash, headerLength)) return;
                ...
                if (!copyToSize(in, stash, frameLength)) return;
            ...
            while (begin < limit) {
                ...
                into.add(
                    unpackFrame(newBytes, begin, begin + frameLength, header)
                );
                begin += frameLength;
            }
        } finally {
