...
import org.apache.cassandra.net.Result.MessagingSuccess;
...
public class OutboundConnection {
    ...
    private static final NoSpamLogger noSpamLogger = NoSpamLogger.getLogger(
        logger,
        30L,
        TimeUnit.SECONDS
    );
    ...
    private static final AtomicLongFieldUpdater<
        OutboundConnection
    > submittedUpdater = AtomicLongFieldUpdater.newUpdater(
        OutboundConnection.class,
        "submittedCount"
    );
    private static final AtomicLongFieldUpdater<
        OutboundConnection
    > pendingCountAndBytesUpdater = AtomicLongFieldUpdater.newUpdater(
        OutboundConnection.class,
        "pendingCountAndBytes"
    );
    private static final AtomicLongFieldUpdater<
        OutboundConnection
    > overloadedCountUpdater = AtomicLongFieldUpdater.newUpdater(
        OutboundConnection.class,
        "overloadedCount"
    );
    private static final AtomicLongFieldUpdater<
        OutboundConnection
    > overloadedBytesUpdater = AtomicLongFieldUpdater.newUpdater(
        OutboundConnection.class,
        "overloadedBytes"
    );
    private static final AtomicReferenceFieldUpdater<
        OutboundConnection,
        Future
    > closingUpdater = AtomicReferenceFieldUpdater.newUpdater(
        OutboundConnection.class,
        Future.class,
        "closing"
    );
    private static final AtomicReferenceFieldUpdater<
        OutboundConnection,
        Future
    > scheduledCloseUpdater = AtomicReferenceFieldUpdater.newUpdater(
        OutboundConnection.class,
        Future.class,
        "scheduledClose"
    );
    ...
    private static boolean isMaxPendingCount(long pendingCountAndBytes) {
        return (
            (pendingCountAndBytes & (-1L << pendingByteBits)) ==
            (-1L << pendingByteBits)
        );
    }
    ...
    private static long pendingCountAndBytes(
        long pendingCount,
        long pendingBytes
    ) {
    ...
    private static class Connecting extends Disconnected {
        ...
        Connecting(
            ...
            Future<Result<MessagingSuccess>> attempt
        ) {
        ...
        Connecting(
            ...
            this.isFailingToConnect =
                scheduled != null ||
                (previous.isConnecting() &&
                    previous.connecting().isFailingToConnect);
        }
    ...
    OutboundConnection(
        ...
        this.delivery = type == ConnectionType.LARGE_MESSAGES
            ? new LargeMessageDelivery(
                template.socketFactory.synchronousWorkExecutor
            )
    ...
    private Outcome acquireCapacity(long count, long bytes) {
        ...
        loop: while (true) {
            ...
            if (pendingBytes(next) <= pendingCapacityInBytes) {
                if (
                    pendingCountAndBytesUpdater.compareAndSet(
                        this,
                        current,
                        next
                    )
            ...
            long requiredReserve = min(
                bytes,
                pendingBytes(next) - pendingCapacityInBytes
            );
            if (unusedClaimedReserve < requiredReserve) {
                long extraGlobalReserve =
                    requiredReserve - unusedClaimedReserve;
                switch (
                    outcome = reserveCapacityInBytes.tryAllocate(
                        extraGlobalReserve
                    )
                ) {
                    case INSUFFICIENT_ENDPOINT:
            ...
            if (
                pendingCountAndBytesUpdater.compareAndSet(this, current, next)
            ) {
        ...
        if (unusedClaimedReserve > 0) reserveCapacityInBytes.release(
            unusedClaimedReserve
        );
    ...
    private void releaseCapacity(long count, long bytes) {
        ...
        if (pendingBytes(prev) > pendingCapacityInBytes) {
            long excess = min(
                pendingBytes(prev) - pendingCapacityInBytes,
                bytes
            );
    ...
    private void onOverloaded(Message<?> message) {
        ...
        noSpamLogger.warn(
            "{} overloaded; dropping {} message (queue: {} local, {} endpoint, {} global)",
            this,
            FBUtilities.prettyPrintMemory(canonicalSize),
            readablePendingBytes,
            readableReserveEndpointUsing,
    ...
    private boolean onExpired(Message<?> message) {
        if (logger.isTraceEnabled()) logger.trace(
            "{} dropping message of type {} with payload {} whose timeout ({}ms) expired before reaching the network. {}ms elapsed after expiration. {}ms since creation.",
            id(),
            message.verb(),
            message.payload,
            DatabaseDescriptor.getRpcTimeout(MILLISECONDS),
            NANOSECONDS.toMillis(
                Clock.Global.nanoTime() - message.expiresAtNanos()
            ),
            message.elapsedSinceCreated(MILLISECONDS)
        );
        else noSpamLogger.warn(
            "{} dropping message of type {} whose timeout expired before reaching the network",
            id(),
            message.verb()
        );
     ...
     */
    private void onFailedSerialize(
        Message<?> message,
        int messagingVersion,
        ...
        logger.warn(
            "{} dropping message of type {} due to error",
            id(),
            message.verb(),
            t
        );
    ...
    private abstract class Delivery extends AtomicInteger implements Runnable {
        ...
        public void execute() {
            if (
                get() < EXECUTE_AGAIN &&
                STOPPED ==
                getAndUpdate(i -> i == STOPPED ? EXECUTING : i | EXECUTE_AGAIN)
        ...
        void executeAgain() {
            ...
            if (
                !isExecuting(
                    getAndUpdate(i ->
                        !isExecuting(i) ? EXECUTING : EXECUTING_AGAIN
                    )
                )
            ) executor.execute(this);
        }
        ...
        private void maybeExecuteAgain() {
            if (
                EXECUTING_AGAIN ==
                getAndUpdate(i ->
                    i == EXECUTING_AGAIN ? EXECUTING : (i & ~EXECUTING)
                )
            ) executor.execute(this);
        }
        ...
        void setInProgress(boolean inProgress) {
            ...
            if (!inProgress && wasInProgress) executeAgain();
        }
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (!doRun(state.established())) break;
            }
    ...
    class EventLoopDelivery extends Delivery {
        ...
        boolean doRun(Established established) {
            ...
            int maxSendBytes = (int) min(
                pendingBytes() - flushingBytes,
                LARGE_MESSAGE_THRESHOLD
            );
            if (maxSendBytes == 0) return false;
            ...
            int sendingCount = 0;
            try (
                OutboundMessageQueue.WithLock withLock = queue.lockOrCallback(
                    approxTime.now(),
                    this::execute
                )
            ) {
                if (withLock == null) return false; // we failed to acquire the queue lock, so return; we will be scheduled again when the lock is available

                sending = established.payloadAllocator.allocate(
                    true,
                    maxSendBytes
                );
                DataOutputBufferFixed out = new DataOutputBufferFixed(
                    sending.buffer
                );
                ...
                while (null != (next = withLock.peek())) {
                    try {
                        ...
                        if (
                            messageSize >
                            DatabaseDescriptor.getInternodeMaxMessageSizeInBytes()
                        ) throw new Message.OversizedMessageException(
                            messageSize
                        );
                        ...
                        Tracing.instance.traceOutgoingMessage(
                            next,
                            messageSize,
                            settings.connectTo
                        );
                        ...
                        if (
                            sending.length() != sendingBytes + messageSize
                        ) throw new InvalidSerializedSizeException(
                            next.verb(),
                            messageSize,
                            sending.length() - sendingBytes
                        );
                ...
                if (0 == sendingBytes) return false;

                sending.finish();
                debug.onSendSmallFrame(sendingCount, sendingBytes);
                ChannelFuture flushResult = AsyncChannelPromise.writeAndFlush(
                    established.channel,
                    sending
                );
                sending = null;
                ...
                if (flushResult.isSuccess()) {
                    ...
                    boolean hasOverflowed =
                        flushingBytes >= settings.flushHighWaterMark;
                    if (hasOverflowed) {
                        isWritable = false;
                        promiseToExecuteLater();
                    }
                    ...
                    flushResult.addListener(future -> {
                        ...
                        if (flushingBytes == 0) setInProgress(false);

                        if (
                            !isWritable &&
                            flushingBytes <= settings.flushLowWaterMark
                        ) {
                            isWritable = true;
                            executeAgain();
                        }
                ...
                if (canonicalSize > 0) releaseCapacity(
                    sendingCount,
                    canonicalSize
                );

                if (sending != null) sending.release();

                if (pendingBytes() > flushingBytes && isWritable) execute();
            }
    ...
    class LargeMessageDelivery extends Delivery {
        ...
        public void run() {
            ...
            try {
                ...
                if (priorThreadName != null) Thread.currentThread()
        ...
        boolean doRun(Established established) {
            ...
            try {
                int messageSize = send.serializedSize(
                ...
                onFailedSerialize(
                    send,
                    established.messagingVersion,
                    out == null ? 0 : (int) out.flushedToNetwork(),
                    t
                );
                return tryAgain;
            }
    ...
    private void invalidateChannel(Established established, Throwable cause) {
        ...
        if (state != established) return; // do nothing; channel already invalidated

        if (isCausedByConnectionReset(cause)) logger.info(
            "{} channel closed by provider",
            id(),
            cause
        );
        else logger.error(
    ...
    Future<?> initiate() {
        class Initiate {
             ...
             */
            long retryRateMillis =
                DatabaseDescriptor.getMinRpcTimeout(MILLISECONDS) / 2;

            // our connection settings, possibly updated on retry
            int messagingVersion = template
                .endpointToVersion()
                .get(template.to);
            OutboundConnectionSettings settings;
            ...
            void onFailure(Throwable cause) {
                if (cause instanceof ConnectException) noSpamLogger.info(
                    "{} failed to connect",
                    id(),
                    cause
                );
                ...
                if (hasPending()) {
                    ...
                    state = Disconnected.dormant(
                        state.disconnected().maintenance
                    );
                }
            }
            ...
            void onCompletedHandshake(Result<MessagingSuccess> result) {
                switch (result.outcome) {
                    case SUCCESS:
                        ...
                        logger.info(
                            "{} successfully connected, version = {}, framing = {}, encryption = {}",
                            id(true),
                            success.messagingVersion,
                            settings.framing,
                    ...
                    case RETRY:
                        if (logger.isTraceEnabled()) logger.trace(
                            "{} incorrect legacy peer version predicted; reconnecting",
                            id()
                        );
                        ...
                        settings.endpointToVersion.set(
                            settings.to,
                            messagingVersion
                        );
                    ...
                    case INCOMPATIBLE:
                        ...
                        Throwable t = new IOException(
                            String.format(
                                "Incompatible peer: %s, messaging version: %s",
                                settings.to,
            ...
            private void attempt(
                ...
                if (knownMessagingVersion != messagingVersion) {
                    logger.trace(
                        "Endpoint version changed from {} to {} since connection initialized, updating.",
                        messagingVersion,
                        knownMessagingVersion
                    );
                ...
                int index = sslFallbackEnabled &&
                    settings.withEncryption() &&
                    settings.encryption.getOptional()
                    ? (int) (connectionAttempts - 1) %
                    fallBackSslFallbackConnectionTypes.length
                    : 0;
                if (
                    fallBackSslFallbackConnectionTypes[index] !=
                    SslFallbackConnectionType.SERVER_CONFIG
                ) {
                    logger.info(
                        "ConnectionId {} is falling back to {} reconnect strategy for retry",
                        id(),
                        fallBackSslFallbackConnectionTypes[index]
                    );
                }
                initiateMessaging(
                    ...
                    if (future.isSuccess()) onCompletedHandshake(
                        //noinspection unchecked
                        (Result<MessagingSuccess>) future.getNow()
                    );
        ...
        runOnEventLoop(() -> {
            if (isClosed()) { // never going to connect
                ...
                if (!state.isConnecting()) {
                    ...
                    state
                        .connecting()
                        .attempt.addListener(new PromiseNotifier<>(promise));
                }
            }
        });
        return promise;
    ...
    Future<Void> reconnectWith(OutboundConnectionSettings reconnectWith) {
        OutboundConnectionSettings newTemplate = reconnectWith.withDefaults(
            ConnectionCategory.MESSAGING
        );
        if (
            newTemplate.socketFactory != template.socketFactory
        ) throw new IllegalArgumentException();
        if (
            newTemplate.callbacks != template.callbacks
        ) throw new IllegalArgumentException();
        if (
            !Objects.equals(
                newTemplate.applicationSendQueueCapacityInBytes,
                template.applicationSendQueueCapacityInBytes
            )
        ) throw new IllegalArgumentException();
        if (
            !Objects.equals(
                newTemplate.applicationSendQueueReserveEndpointCapacityInBytes,
                template.applicationSendQueueReserveEndpointCapacityInBytes
            )
        ) throw new IllegalArgumentException();
        if (
            newTemplate.applicationSendQueueReserveGlobalCapacityInBytes !=
            template.applicationSendQueueReserveGlobalCapacityInBytes
        ) throw new IllegalArgumentException();
        ...
        return runOnEventLoop(() -> {
            if (state == closeIfIs) {
                ...
                closeIfIs.channel
                    .close()
                    .addListener(future -> {
                        if (!future.isSuccess()) logger.info(
                            "Problem closing channel {}",
                            closeIfIs,
                            future.cause()
                        );
                    });
    ...
    Future<Void> scheduleClose(long time, TimeUnit unit, boolean flushQueue) {
        ...
        if (
            !scheduledCloseUpdater.compareAndSet(this, null, scheduledClose)
        ) return this.scheduledClose;

        eventLoop.schedule(
            () ->
                close(flushQueue).addListener(
                    new PromiseNotifier<>(scheduledClose)
                ),
            time,
            unit
        );
    ...
    public Future<Void> close(boolean flushQueue) {
        ...
        if (
            !closingUpdater.compareAndSet(this, null, closing)
        ) return this.closing;
        ...
        if (flushQueue) {
            ...
            class FinishDelivery implements Runnable {
                ...
                public void run() {
                    if (!hasPending()) delivery.stopAndRunOnEventLoop(
                        eventLoopCleanup
                    );
                    else delivery.stopAndRun(() -> {
                        if (
                            state.isConnecting() &&
                            state.connecting().isFailingToConnect
                        ) clearQueue.run();
                        run();
                    });
                }
    ...
    private String id(boolean includeReal) {
        ...
        OutboundConnectionSettings settings = established.settings;
        return SocketFactory.channelId(
            settings.from,
    ...
    int messagingVersion() {
        ...
        return state.isEstablished()
            ? state.established().messagingVersion
