...
// import static org.apache.cassandra.net.OutboundConnectionInitiator.Result.streamingSucces
...
// import org.apache.cassandra.net.ConnectionType.STREAMING;
import org.apache.cassandra.net.HandshakeProtocol.Accept;
import org.apache.cassandra.net.HandshakeProtocol.Initiate;
// import org.apache.cassandra.net.Result.MILLISECONDS;
import org.apache.cassandra.net.Result.MessagingSuccess;
// import org.apache.cassandra.net.Result.STREAMING;
import org.apache.cassandra.net.Result.StreamingSuccess;
...
public class OutboundConnectionInitiator<SuccessType extends Result.Success> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(
        OutboundConnectionInitiator.class
    );
    ...
    private OutboundConnectionInitiator(
        ConnectionType type,
     ...
     */
    public static Future<Result<StreamingSuccess>> initiateStreaming(
        EventLoop eventLoop,
        OutboundConnectionSettings settings,
        SslFallbackConnectionType sslConnectionType
    ) {
        return new OutboundConnectionInitiator<StreamingSuccess>(
            STREAMING,
            sslConnectionType,
            settings,
     ...
     * The returned {@code Future} is guaranteed to be completed on the supplied eventLoop.
     */
    static Future<Result<MessagingSuccess>> initiateMessaging(
        EventLoop eventLoop,
        ConnectionType type,
        SslFallbackConnectionType sslConnectionType,
        OutboundConnectionSettings settings,
        ...
        return new OutboundConnectionInitiator<>(
            type,
            sslConnectionType,
            settings,
            result
    ...
    private Future<Result<SuccessType>> initiate(EventLoop eventLoop) {
        ...
        if (
            !settings.authenticator.authenticate(
                settings.to.getAddress(),
                settings.to.getPort(),
                null,
                OUTBOUND_PRECONNECT
            )
            ...
            return ImmediateFuture.failure(
                new IOException(
                    "Authentication failed to " + settings.connectToId()
                )
            );
        }
        ...
        io.netty.util.concurrent.Future<Void> bootstrap = createBootstrap(
            eventLoop
        )
            .connect()
            .addListener(future -> {
                eventLoop.execute(() -> {
                    if (!future.isSuccess()) {
                        if (
                            future.isCancelled() && !timedout.get()
                        ) resultPromise.cancel(true);
                        else if (future.isCancelled()) resultPromise.tryFailure(
                            new IOException(
                                "Timeout handshaking with " +
    ...
    private Bootstrap createBootstrap(EventLoop eventLoop) {
        Bootstrap bootstrap = settings.socketFactory
            ...
            .option(
                ...
                settings.tcpConnectTimeoutInMS
            )
            ...
            .option(
                ...
                NoSizeEstimator.instance
            )
            .handler(new Initializer());

        if (settings.socketSendBufferSizeInBytes > 0) bootstrap.option(
            ChannelOption.SO_SNDBUF,
            settings.socketSendBufferSizeInBytes
        );
    ...
    private class Initializer extends ChannelInitializer<SocketChannel> {
        ...
        public void initChannel(SocketChannel channel) throws Exception {
            ...
            if (
                (sslConnectionType == SslFallbackConnectionType.SERVER_CONFIG &&
                ...
                InetSocketAddress peer =
                    settings.encryption.require_endpoint_verification
                        ? new InetSocketAddress(
                            address.getAddress(),
                            address.getPort()
                        )
                        : null;
                SslHandler sslHandler = newSslHandler(
                    channel,
                    sslContext,
                    peer
                );
                if (logger.isTraceEnabled()) logger.trace(
                    "creating outbound netty SslContext: context={}, engine={}",
            ...
            pipeline.addLast(
                ...
                new ServerAuthenticationHandler(settings)
            );

            if (WIRETRACE) pipeline.addLast(
                "logger",
        ...
        private SslContext getSslContext(
            ...
            EncryptionOptions.ClientEncryptionOptions.ClientAuth requireClientAuth =
                NOT_REQUIRED;
            if (connectionType == SslFallbackConnectionType.MTLS) {
                ...
                connectionType == SslFallbackConnectionType.SERVER_CONFIG
            ) {
                requireClientAuth = settings.withEncryption()
                    ? REQUIRED
                    : NOT_REQUIRED;
            }
            return SSLFactory.getOrCreateSslContext(
                settings.encryption,
                requireClientAuth,
    ...
    static class ServerAuthenticationHandler extends ByteToMessageDecoder {
        ...
        protected void decode(
            ...
            final Certificate[] certificates = certificates(
            ...
            if (
                ...
                channelHandlerContext
                    ...
                    .replace(
                        this,
                        DISCARD_HANDLER_NAME,
                        new InternodeConnectionUtils.ByteBufDiscardHandler()
                    );
    ...
    private class Handler extends ByteToMessageDecoder {
        ...
        public void channelActive(final ChannelHandlerContext ctx)
            throws Exception {
            final Initiate msg = new Initiate(
            ...
            logger.trace(
                "starting handshake with peer {}, msg = {}",
                settings.connectToId(),
                msg
            );
        ...
        protected void decode(
            ...
            try {
                final Accept msg = Accept.maybeDecode(in);
                if (msg == null) return;

                final int useMessagingVersion = msg.useMessagingVersion;
                final int peerMessagingVersion = msg.maxMessagingVersion;
                logger.trace(
                    "received second handshake message from peer {}, msg = {}",
                    settings.connectTo,
                    msg
                );
                ...
                if (
                    useMessagingVersion < settings.acceptVersions.min ||
                    ...
                    if (type.isMessaging()) {
                        ...
                        result = (Result<SuccessType>) Result.messagingSuccess(
                            ctx.channel(),
                            useMessagingVersion,
                            frameEncoder.allocator()
                        );
                    } else {
                        result = (Result<SuccessType>) Result.streamingSuccess(
                            ctx.channel(),
                            useMessagingVersion
                        );
                    }
                ...
                if (result.isSuccess()) {
                    BufferPools.forNetworking()
                        .setRecycleWhenFreeForCurrentThread(false);
                    if (type.isMessaging()) {
                ...
                if (
                    !resultPromise.trySuccess(result) && result.isSuccess()
