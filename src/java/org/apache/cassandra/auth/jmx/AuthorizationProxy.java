...
public class AuthorizationProxy implements InvocationHandler {
    ...
    public static final JmxPermissionsCache jmxPermissionsCache =
        new JmxPermissionsCache();
     ...
     */
    protected Function<ObjectName, Set<ObjectName>> queryNames = name ->
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
        ...
        try {
            return null;
            // if (
            //     "getMBeanServer".equals(methodName)
            ...
            //     if (subject != null) throw new SecurityException(
            //         "Access denied"
            //     );
    ...
    public boolean authorize(
        ...
        if (logger.isTraceEnabled()) logger.trace(
            "Authorizing JMX method invocation {} for {}",
            methodName,
        ...
        // RoleResource userResource = RoleResource.role(
     ...
     */
    private boolean authorizeMBeanServerMethod(
        RoleResource subject,
        String methodName
    ) {
        logger.trace(
     ...
     * @throws SecurityException if authorization fails
     */
    private boolean authorizeMBeanMethod(
        RoleResource role,
        String methodName,
        Object[] args
    ) {
        ...
        if (logger.isTraceEnabled()) logger.trace(
            "JMX invocation of {} on {} requires permission {}",
            methodName,
            targetBean,
            requiredPermission
        );
        ...
        Set<JMXResource> permittedResources = getPermittedResources(
            role,
            requiredPermission
        );
     ...
     */
    private Set<JMXResource> getPermittedResources(
        RoleResource subject,
        Permission required
    ) {
        ...
        return getPermissions
            ...
            .anyMatch(
                details ->
                    ...
                    details.resource.equals(resource)
            );
    }
    ...
    private boolean checkPattern(
        ObjectName target,
        Set<JMXResource> permittedResources
    ) {
        ...
        for (JMXResource resource : permittedResources) {
            try {
                Set<ObjectName> matchingNames = queryNames.apply(
                    ObjectName.getInstance(resource.getObjectName())
                );
        ...
        logger.trace(
            "Subject does not have sufficient permissions on all MBeans matching the target pattern {}",
            target
        );
        return false;
    }
    ...
    private boolean checkExact(
        ...
        for (JMXResource resource : permittedResources) {
            try {
                if (
                    ObjectName.getInstance(resource.getObjectName()).apply(
                        target
                    )
                ) return true;
            } catch (MalformedObjectNameException e) {
        ...
        logger.trace(
            "Subject does not have sufficient permissions on target MBean {}",
            target
        );
        return false;
    }
    ...
    private static Permission getRequiredPermission(String methodName) {
        switch (methodName) {
            ...
            default:
                logger.debug(
                    "Access denied, method name {} does not map to any defined permission",
                    methodName
                );
                return null;
        }
     ...
     *         JMXResources
     */
    private static Set<PermissionDetails> loadPermissions(
        RoleResource subject
    ) {
        ...
        return DatabaseDescriptor.getAuthorizer()
            .list(AuthenticatedUser.SYSTEM_USER, Permission.ALL, null, subject)
            .stream()
    ...
    private void checkCompilerDirectiveAddMethods(
        ObjectName name,
        String operation
    ) {
        if (
    ...
    private void checkMLetMethods(ObjectName name, String operation) {
        ...
        try {
            if (
                !mbs.isInstanceOf(name, "javax.management.loading.MLet")
            ) return;
    ...
    public static final class JmxPermissionsCache
        ...
        protected JmxPermissionsCache() {
            ...
            MBeanWrapper.instance.registerMBean(
                this,
                MBEAN_NAME_BASE + DEPRECATED_CACHE_NAME
            );
        }
        ...
        protected void unregisterMBean() {
            ...
            MBeanWrapper.instance.unregisterMBean(
                MBEAN_NAME_BASE + DEPRECATED_CACHE_NAME,
