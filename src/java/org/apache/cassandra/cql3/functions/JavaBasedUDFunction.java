...
public final class JavaBasedUDFunction extends UDFunction {
    ...
    private static final UDFExecutorService executor = new UDFExecutorService(
        new NamedThreadFactory(
            "UserDefinedFunctions",
            Thread.MIN_PRIORITY,
            udfClassLoader,
            new SecurityThreadGroup(
                "UserDefinedFunctions",
                null,
    ...
    static {
        udfByteCodeVerifier.addDisallowedMethodCall(
            "java/lang/Class",
            "forName"
        );
        udfByteCodeVerifier.addDisallowedMethodCall(
        ...
        try (
            InputStream input =
                JavaBasedUDFunction.class.getResource("JavaSourceUDF.txt")
                    .openConnection()
                    .getInputStream()
        ) {
            ...
            for (int i = 0; st.hasMoreElements(); i++) javaSourceTemplate[i] =
                st.nextToken();
        } catch (IOException e) {
    ...
    private static final Pattern patternJavaDriver = Pattern.compile(
    ...
    JavaBasedUDFunction(
        ...
        try {
            EcjCompilationUnit compilationUnit = new EcjCompilationUnit(
                javaSource,
                targetClassName
            );
            ...
            if (
                compilationUnit.problemList != null &&
                !compilationUnit.problemList.isEmpty()
            ) {
                ...
                for (IProblem problem : compilationUnit.problemList) {
                    ...
                    if (ln < 1L) {
                        if (problem.isError()) {
                            ...
                            problems
                                .append("GENERATED SOURCE ERROR: line ")
                ...
                if (fullSource) throw new InvalidRequestException(
            ...
            Set<String> errors = udfByteCodeVerifier.verify(
                targetClassName,
                targetClassLoader.classData(targetClassName)
            );
            String validDeclare =
                "not allowed method declared: " + executeInternalName + '(';
            ...
            if (!errors.isEmpty()) throw new InvalidRequestException(
                "Java UDF validation failed: " + errors
            );
            ...
            try {
                ...
                MethodHandle ctor = MethodHandles.lookup()
                    .findConstructor(cls, methodType);
                this.javaUDF = (JavaUDF) ctor.invokeWithArguments(
                    resultType,
                    udfContext
                );
            } finally {
            ...
            logger.error(
                String.format(
                    "Could not compile function '%s' from Java source:%n%s",
                    name,
                    javaSource
                ),
                e
    ...
    @Override
    protected Object executeAggregateUserDefined(
        Object state,
        Arguments arguments
    ) {
    ...
    private static int countNewlines(StringBuilder javaSource) {
        int ln = 0;
        for (int i = 0; i < javaSource.length(); i++) if (
            javaSource.charAt(i) == '\n'
        ) ln++;
        return ln;
    }
    ...
    private static String generateArgumentList(
        ...
        for (int i = 0; i < argTypes.size(); i++) {
            if (i > 0) code.append(", ");
            code
                .append(argTypes.get(i).getJavaTypeName())
                .append(' ')
    ...
    private static String generateArguments(
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (forAggregate && i == 0) {
                ...
                appendGetMethodName(code, argType)
                    .append('(')
                    .append(forAggregate ? i - 1 : i)
                    .append(')');
            }
        }
        return code.toString();
     ...
     * @return the builder
     */
    private static StringBuilder appendGetMethodName(
        StringBuilder code,
        UDFDataType type
    ) {
        code.append("get");
        if (!type.isPrimitive()) return code;

        return code
            .append("As")
            .append(StringUtils.capitalize(type.getJavaTypeName()));
    }
    ...
    static final class EcjCompilationUnit
        ...
        public char[][] getPackageName() {
            ...
            for (int i = 0; i < result.length; i++) result[i] = izer
                .nextToken()
                .toCharArray();
            return result;
        }
        ...
        public void acceptResult(CompilationResult result) {
            if (result.hasErrors()) {
                ...
                for (ClassFile classFile : classFiles) targetClassLoader.addClass(
                    className,
                    classFile.getBytes()
                );
            }
        ...
        private NameEnvironmentAnswer findType(String className) {
            ...
            try (
                InputStream is = UDFunction.udfClassLoader.getResourceAsStream(
                    resourceName
                )
            ) {
                if (is != null) {
        ...
        public boolean isPackage(
            ...
            StringBuilder result = new StringBuilder();
            int i = 0;
            if (parentPackageName != null) for (
                ;
                i < parentPackageName.length;
                i++
            ) {
                if (i > 0) result.append('.');
            ...
            if (
                Character.isUpperCase(packageName[0]) &&
    ...
    static final class EcjTargetClassLoader extends SecureClassLoader {
        ...
        protected Class<?> findClass(String name)
            ...
            // if (classData != null) return defineClass(
            //     name,
            //     classData,
            //     0,
            //     classData.length,
