...
public class CassandraDaemon {
    ...
    public static boolean SKIP_GC_INSPECTOR =
        CassandraRelevantProperties.SKIP_GC_INSPECTOR.getBoolean();
    ...
    static {
        ...
        SharedMetricRegistries.getOrCreate("logback-metrics").addListener(
    ...
    private void maybeInitJmx() {
        ...
        if (COM_SUN_MANAGEMENT_JMXREMOTE_PORT.isPresent()) {
            logger.warn(
                "JMX settings in cassandra.yaml or cassandra-env.sh have been bypassed as the JMX connector server is " +
        ...
        if (!jmxServerOptions.enabled) return;
            ...
            Keyspace.allExisting()
                .forEach(k ->
                    k
                        ...
                        .forEach(ColumnFamilyStore::updateSpeculationThreshold)
                );
        } catch (Throwable t) {
    ...
    protected void setup() {
        ...
        try {
            ...
            exitOrFail(
                StartupException.ERR_WRONG_DISK_STATE,
                e.getMessage(),
                e
            );
        }
        ...
        try {
            ...
            if (
                ClusterMetadata.current().myNodeId() != null
            ) RegistrationStatus.instance.onRegistration();
        } catch (InterruptedException | ExecutionException | IOException e) {
            throw new AssertionError(
                "Can't initialize cluster metadata service",
                e
            );
        ...
        // metadata.
        SystemKeyspace.updateRack(
            ClusterMetadata.current().locator.local().rack
        );
        ...
        ScheduledExecutors.optionalTasks.scheduleWithFixedDelay(
            ColumnFamilyStore.getBackgroundCompactionTaskSubmitter(),
            5,
            1,
            TimeUnit.MINUTES
        );
        ...
        if (
            DatabaseDescriptor.getAuthCacheWarmingEnabled()
        ) AuthCacheService.instance.warmCaches();
        else logger.info("Prewarming of auth caches is disabled");
    ...
    public void migrateSystemDataIfNeeded() throws IOException {
        ...
        Path target = File.getPath(
            DatabaseDescriptor.getLocalSystemKeyspacesDataFileLocations()[0]
        );
        ...
        for (String source : sources) {
            ...
            if (!Files.exists(dataFileLocation)) continue;

            try (Stream<Path> locationChildren = Files.list(dataFileLocation)) {
                Path[] keyspaceDirectories = locationChildren
                    .filter(p ->
                        SchemaConstants.isLocalSystemKeyspace(
                ...
                for (Path keyspaceDirectory : keyspaceDirectories) {
                    try (
                        Stream<Path> keyspaceChildren = Files.list(
                            keyspaceDirectory
                        )
                    ) {
                        Path[] tableDirectories = keyspaceChildren
                            ...
                            .filter(p ->
                                SystemKeyspace.TABLES_SPLIT_ACROSS_MULTIPLE_DISKS.stream()
                                    .noneMatch(t ->
                                        p
                                            .getFileName()
                                            .toString()
                        ...
                        for (Path tableDirectory : tableDirectories) {
                            FileUtils.moveRecursively(
                                tableDirectory,
                                target.resolve(
                                    dataFileLocation.relativize(tableDirectory)
                                )
                            );
                        }
                        ...
                        if (
                            !SchemaConstants.SYSTEM_KEYSPACE_NAME.equals(
                                keyspaceDirectory.getFileName().toString()
                            )
                        ) {
    ...
    public static void disableAutoCompaction(Collection<String> keyspaces) {
        for (String keyspaceName : keyspaces) {
            ...
            for (ColumnFamilyStore cfs : Keyspace.open(
                keyspaceName
            ).getColumnFamilyStores()) {
                ...
                for (final ColumnFamilyStore store : cfs.concatWithIndexes()) {
                    ...
                    if (
                        ...
                        if (
                            ...
                            logger.info(
                                "Not enabling compaction for {}.{}; autocompaction_on_startup_enabled is set to false",
                                store.getKeyspaceName(),
                                store.name
                            );
                        }
                    }
    ...
    public void setupVirtualKeyspaces() {
        VirtualKeyspaceRegistry.instance.register(
            VirtualSchemaKeyspace.instance
        );
        VirtualKeyspaceRegistry.instance.register(SystemViewsKeyspace.instance);
        VirtualKeyspaceRegistry.instance.register(
            new VirtualKeyspace(VIRTUAL_METRICS, createMetricsKeyspaceTables())
        );

        if (
            DatabaseDescriptor.getAccord().enable_virtual_debug_only_keyspace
        ) VirtualKeyspaceRegistry.instance.register(
        ...
        LoggingSupportFactory.getLoggingSupport()
            .getAppender(
                SlowQueriesAppender.class,
                SlowQueriesAppender.APPENDER_NAME
            )
            .ifPresent(appender ->
                appender.flushBuffer(
                    SlowQueriesTable.class,
    ...
    public synchronized void initializeClientTransports() {
        // Native transport
        if (nativeTransportService == null) nativeTransportService =
            new NativeTransportService();
    }
    ...
    private Future<?> loadRowAndKeyCacheAsync() {
        final Future<Integer> keyCacheLoad =
            CacheService.instance.keyCache.loadSavedAsync();

        final Future<Integer> rowCacheLoad =
            CacheService.instance.rowCache.loadSavedAsync();

        @SuppressWarnings("unchecked")
        Future<List<Integer>> retval = FutureCombiner.allOf(
    ...
    public static void logSystemInfo(Logger logger) {
        if (logger.isInfoEnabled()) {
            ...
            logger.info(
                "Heap size: {}/{}",
                FBUtilities.prettyPrintMemory(
                    Runtime.getRuntime().totalMemory()
                ),
                FBUtilities.prettyPrintMemory(Runtime.getRuntime().maxMemory())
            );

            for (MemoryPoolMXBean pool : ManagementFactory.getMemoryPoolMXBeans()) logger.info(
                "{} {}: {}",
                pool.getName(),
                pool.getType(),
                pool.getPeakUsage()
    ...
    public void start() {
        StartupClusterConnectivityChecker connectivityChecker =
            StartupClusterConnectivityChecker.create(
    ...
    private void startClientTransports() {
        ...
        if (
            START_NATIVE_TRANSPORT.getBoolean() ||
    ...
    public void activate(boolean closeStdOutErr) {
        ...
        try {
            ...
            boolean logStackTrace = e instanceof ConfigurationException
                ? ((ConfigurationException) e).logStackTrace
                : true;
    ...
    public static void registerNativeAccess()
        ...
        MBeanWrapper.instance.registerMBean(
            new StandardMBean(new NativeAccess(), NativeAccessMBean.class),
            MBEAN_NAME,
    ...
    public void validateTransportsCanStart() {
        ...
        MultiStepOperation<?> startupSequence =
            metadata.inProgressSequences.get(metadata.myNodeId());
        ...
        if (startupSequence != null) {
            if (StorageService.instance.isSurveyMode()) {
                if (
                    !StorageService.instance.readyToFinishJoiningRing() ||
                    DatabaseDescriptor.getAuthenticator()
                        .requireAuthentication()
                ) {
                    throw new IllegalStateException(
                        "Not starting client transports in write_survey mode as it's bootstrapping or " +
                        "auth is enabled"
                    );
                }
            } else {
            ...
            if (
                StorageService.instance.isReplacingSameAddress() &&
                StorageService.instance.isSurveyMode()
            ) return;

            // This node has not joined the ring (i.e. it was started with -Dcassandra.join_ring=false)
            if (StorageService.instance.isStarting()) return;

            if (!SystemKeyspace.bootstrapComplete()) {
                throw new IllegalStateException(
                    "Node is not yet bootstrapped completely. Use nodetool to check bootstrap" +
    ...
    public void startNativeTransport() {
        ...
        if (nativeTransportService == null) throw new IllegalStateException(
        ...
        if (!alreadyRunning) StorageService.instance.setRpcReady(true);
    }
    ...
    public void stopNativeTransport(boolean force) {
        if (nativeTransportService != null) nativeTransportService.stop(force);
    }

    public boolean isNativeTransportRunning() {
        return (
            nativeTransportService != null && nativeTransportService.isRunning()
        );
    }
