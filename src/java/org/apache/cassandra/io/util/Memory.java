...
public class Memory implements AutoCloseable, ReadableMemory {
    ...
    private static final long BYTE_ARRAY_BASE_OFFSET = unsafe.arrayBaseOffset(
        byte[].class
    );
    ...
    protected Memory(long bytes) {
        ...
        if (peer == 0) throw new OutOfMemoryError(); // checkstyle: permit this instantiation
    }
    ...
    public static Memory allocate(long bytes) {
        ...
        if (Ref.TRACE_ENABLED) return new SafeMemory(bytes);
    ...
    public void setBytes(long memoryOffset, ByteBuffer buffer) {
        ...
        if (buffer.hasArray()) {
            ...
            unsafe.copyMemory(
                MemoryUtil.getAddress(buffer) + buffer.position(),
                peer + memoryOffset,
                buffer.remaining()
            );
    ...
    public void setBytes(
        ...
        unsafe.copyMemory(
            buffer,
            BYTE_ARRAY_BASE_OFFSET + bufferOffset,
            null,
            peer + memoryOffset,
            count
        );
    ...
    public void getBytes(
        ...
        checkBounds(memoryOffset, memoryOffset + count);
        FastByteOperations.UnsafeOperations.copy(
            null,
            peer + memoryOffset,
            buffer,
            bufferOffset,
    ...
    protected void checkBounds(long start, long end) {
        ...
        assert start >= 0 && end <= size && start <= end : "Illegal bounds [" +
        start +
        ".." +
        end +
        "); size: " +
    ...
    public boolean equals(Object o) {
        ...
        Memory b = (Memory) o;
        if (peer == b.peer && size == b.size) return true;
        return false;
    }
    ...
    public ByteBuffer[] asByteBuffers(long offset, long length) {
        ...
        ByteBuffer[] result = new ByteBuffer[(int) (length /
            Integer.MAX_VALUE) +
        1];
        int size = (int) (size() / result.length);
        for (int i = 0; i < result.length - 1; i++) {
            result[i] = LittleEndianMemoryUtil.getByteBuffer(
                peer + offset,
                size
            );
            offset += size;
        ...
        result[result.length - 1] = LittleEndianMemoryUtil.getByteBuffer(
            peer + offset,
            (int) length
        );
        return result;
