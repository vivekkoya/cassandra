...
import org.apache.cassandra.net.Result;
import org.apache.cassandra.net.Result.*;
...
public class NettyStreamingConnectionFactory
    ...
    public static NettyStreamingChannel connect(
        OutboundConnectionSettings template,
        int messagingVersion,
        StreamingChannel.Kind kind
    ) throws IOException {
        EventLoop eventLoop = MessagingService.instance()
            .socketFactory.outboundStreamingGroup()
            .next();
        OutboundConnectionSettings settings = template.withDefaults(
            ConnectionCategory.STREAMING
        );
        ...
        for (final SslFallbackConnectionType sslFallbackConnectionType : sslFallbacks) {
            for (int i = 0; i < MAX_CONNECT_ATTEMPTS; i++) {
                Future<Result<StreamingSuccess>> result = initiateStreaming(
                    eventLoop,
                    settings,
        ...
        throw new IOException(
            "failed to connect to " + template.to + " for streaming data",
            cause
        );
    }

    @Override
    public StreamingChannel create(
        InetSocketAddress to,
        int messagingVersion,
        StreamingChannel.Kind kind
    ) throws IOException {
        return connect(
            new OutboundConnectionSettings(getByAddress(to)),
            messagingVersion,
            kind
        );
    }
    ...
    public StreamingChannel create(
        ...
        return connect(
            new OutboundConnectionSettings(
                getByAddress(to),
                getByAddress(preferred)
            ),
            messagingVersion,
            kind
        );
